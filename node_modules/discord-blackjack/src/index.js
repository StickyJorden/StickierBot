...
module.exports = async (message, client, options) => {
    ...
    try {
        ...
        let hitButton = new disbut.MessageButton()
            .setLabel("Hit")
            .setID("hit")
            .setStyle("blurple");
    
        let standButton = new disbut.MessageButton()
            .setLabel("Stand")
            .setID("stand")
            .setStyle("blurple");
        
        let doubleButton = new disbut.MessageButton()
            .setLabel("Double Down")
            .setID("double")
            .setStyle("blurple");

        let splitButton = new disbut.MessageButton()
            .setLabel("Split")
            .setID("split")
            .setStyle("blurple");
        ...
        if (addco != "Soft ") {
            ...
            if (yourdeck[0].value == yourdeck[1].value) {
                ...
                row = new disbut.MessageActionRow()
                    .addComponents(hitButton, standButton, splitButton);
            }
            else if (1 == 1) {
                content = doubledown
                filter = filter2
                row = new disbut.MessageActionRow()
                    .addComponents(hitButton, standButton, doubleButton);
            }
        }

        
        if (RESULTS == "Unknown") {
            let ori = message.channel.send({ content: content, embed: normalembed/* , components: row */})
                    ...
                    if (!allresponses.size) {
                        ...
                        if (["h", "hit", "hi"].includes(theanswer)) {
                            ...
                            if (responsenow != "s") {
                                ...
                                ori = message.channel.send({ content: normalcontent, embed: normalembed/* , components: row */ })
                                normalembed.fields[0].value = normalembed.fields[0].value.replace(copiedEmbed.value, `{yvalue}`)
                            }
                            startAt++
