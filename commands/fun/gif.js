...
module.exports = {
	...
	async execute(interaction, message, args) {
      ...
      if(term == undefined)
      {
        message.channel.send({content: `I need something to search please. Usage !gif <subject>`});
	  }
	  else if(term == "<@!423837609081700352>")
	  {
		message.channel.send({content: `https://tenor.com/view/hold-on-laughing-this-guy-asimp-black-this-nigga-asimp-gif-16283123`});
	  }
	  else if(term == "<@423837609081700352>")
	  {
		message.channel.send({content: `https://tenor.com/view/hold-on-laughing-this-guy-asimp-black-this-nigga-asimp-gif-16283123`});
      ...
      // strip off 'gif' from the message and that'll be the search term
        ...
        // reply to the channel by sending a message (image url)
          ...
          */
			...
			{
				message.channel.send({content: "Couldn't quite find that. Search something else?"});
			}
			else
			{
				message.channel.send({content: content.data[item].images.fixed_height.url});
			}
		  }
		})
	}
